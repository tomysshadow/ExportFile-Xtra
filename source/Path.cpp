...
std::string Path::Info::toRelativePath(const std::string &path, unsigned long productVersionMajor) {
	...
	std::string relativePath = FILESYSTEM_DIRECTOR_PATH(path, productVersionMajor).lexically_relative(std::filesystem::current_path()).string();
...
bool Path::Info::getDirnameOptional(std::optional<std::string> &dirnameOptional) {
	...
	if (dirnameOptional.has_value()) {
		...
		std::string dirname = FILESYSTEM_DIRECTOR_PATH(dirnameOptional.value(), productVersionMajor).lexically_normal().string();
