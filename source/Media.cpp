...
namespace Media {
	void DirectorMedia::Content::destroy() {
	...
	*/
	...
	/*
	DirectorMedia::Content::Content(const Content &content) {
		duplicate(content);
	}

	DirectorMedia::Content &DirectorMedia::Content::operator=(const Content &content) {
		if (this == &content) {
			return *this;
		}
	...
	*/
	...
	PIMoaReader DirectorMedia::Content::getReaderInterfacePointer() const {
		return (PIMoaReader)getInterface(readerInterfacePointer);
	}

	PIMoaRegistryEntryDict DirectorMedia::Content::getReaderRegistryEntryDictInterfacePointer() const {
		return (PIMoaRegistryEntryDict)getInterface(readerRegistryEntryDictInterfacePointer);
	}

	PIMoaDataObject DirectorMedia::Content::getDataObjectInterfacePointer() const {
		return (PIMoaDataObject)getInterface(dataObjectInterfacePointer);
	}

	void DirectorMedia::Content::setReaderInterfacePointer(PIMoaReader readerInterfacePointer) {
	...
	MixerMedia::MixerMedia(const MixerMedia &mixerMedia)
		: exportFileValueConverter(mixerMedia.exportFileValueConverter),
		lingo(mixerMedia.lingo) {
		duplicate(mixerMedia);
	}

	MixerMedia &MixerMedia::operator=(const MixerMedia &mixerMedia) {
		if (this == &mixerMedia) {
			return *this;
		}
	...
	PIMoaDrMovieContext MixerMedia::getDrMovieContextInterfacePointer() const {
		return (PIMoaDrMovieContext)getInterface(drMovieContextInterfacePointer);
	}

	PIMoaDrCastMem MixerMedia::getDrCastMemInterfacePointer() const {
		return (PIMoaDrCastMem)getInterface(drCastMemInterfacePointer);
	}

	PIMoaRegistryEntryDict MixerMedia::getRegistryEntryDictInterfacePointer() const {
		return (PIMoaRegistryEntryDict)getInterface(registryEntryDictInterfacePointer);
	}

	PIMoaMmValue MixerMedia::getMmValueInterfacePointer() const {
		return (PIMoaMmValue)getInterface(mmValueInterfacePointer);
	}

	MoaMmSymbol MixerMedia::getIsSavingSymbol() const {
	...
	MixerMedia::Lingo::Lingo(const Lingo &lingo) {
		duplicate(lingo);
	}

	MixerMedia::Lingo &MixerMedia::Lingo::operator=(const Lingo &lingo) {
		if (this == &lingo) {
			return *this;
		}
	...
	bool WinBMPMedia::rgbX(RGBTRIPLE* rgbTriplePointer, DWORD stride, DWORD imageSize) {
		if (!rgbTriplePointer) {
			return false;
		}
	...
	bool WinBMPMedia::rgbaX(RGBQUAD* rgbQuadPointer, DWORD stride, DWORD imageSize) {
		if (!rgbQuadPointer) {
			return false;
		}
	...
	int WinBMPMedia::getCoordinate(int dimension) {
