...
namespace Asset {
	std::string& trimEllipsis(std::string &displayName) {
		...
		static constexpr std::string::size_type ELLIPSIS_SIZE = 3;
	...
	MoaError Assets::Info::createIconAssetStream(
		...
		if (createdNew) {
			*streamOptionalPointer = Stream(callbackInterfacePointer);
		}
	...
	*/
	...
	Assets::Info::Info(const Info &info) {
		duplicate(info);
	}

	Assets::Info &Assets::Info::operator=(const Info &info) {
		if (this == &info) {
			return *this;
		}
